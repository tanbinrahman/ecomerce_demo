          ...
          <aside class="aa-sidebar">
            ...
            <!-- single sidebar -->
              ...
              <ul class="aa-catg-nav">
                @foreach($categories as $left_cat)
                  @if($slug==$left_cat->category_slug)
                    <li><a class="left_cat_active" href="{{url('category/'.$left_cat->category_slug)}}">{{$left_cat->category_name}}</a></li>
                  @else
                  <li><a  href="{{url('category/'.$left_cat->category_slug)}}">{{$left_cat->category_name}}</a></li>
                  @endif
                @endforeach
              </ul>
            </div>
            ...
            <div class="aa-sidebar-widget">
              ...
              <div class="aa-color-tag">
                @foreach($colors as $color)
                  @if(in_array($color->id,$colorFilterAttr ))
                    <a class="aa-color-{{strtolower($color->color)}} active_color"  href="javascript:void(0)" onclick="set_colot('{{$color->id}}','1')"></a>
                  @else
                    <a class="aa-color-{{strtolower($color->color)}}"  href="javascript:void(0)" onclick="set_colot('{{$color->id}}','0')"></a>
                  @endif
                @endforeach
              </div>                            
            </div>
